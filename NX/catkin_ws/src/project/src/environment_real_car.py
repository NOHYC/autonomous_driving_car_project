...
from std_msgs.msg import Float32MultiArray # add
...
class Env():
    def __init__(self, action_dim=2):
        ...
        self.sub_red = rospy.Subscriber('/TFF/RED_Emergency_Stop', Bool, self.red_call)
        self.sub_yellow = rospy.Subscriber('/TFF/YELLOW_Normal_Stop', Bool, self.yellow_call)
        self.sub_green = rospy.Subscriber('/TFF/GREEN_Start_DDPG', Bool, self.green_call)     
        self.pub = rospy.Publisher('/vesc/low_level/ackermann_cmd_mux/input/teleop', AckermannDriveStamped, queue_size=5)
        rospy.Subscriber("/TFF/arduino_imu",Imu,self.get_imu)
        rospy.Subscriber("/TFF/ddpg_camera_input", Float32MultiArray, self.array_callback)
        self.mm_angle = MovingAverage(3)
        self.mm_vel = MovingAverage(3)
    ...
    def update_velocity(self, data):
        self.vehicle_velocity = data.data
    ...
    def red_call(self, data) :
        self.red_button = data.data

    def yellow_call(self, data) :
        self.yellow_button = data.data

    def green_call(self, data) :
    ...
    def setReward(self, state, done):
        ...
        if self.red_button == False:
            done = True

        if self.yellow_button == False:
            done = True
            done_flag = 1
        if done:
            if done_flag != 1: 
    ...
    def step(self, action, past_action):
        ...
        vel_cmd.drive.speed = wmm_vel * linear_param;
        vel_cmd.drive.acceleration = 0;
        vel_cmd.drive.jerk = 0;
        vel_cmd.drive.steering_angle = ang_vel * ang_param
        vel_cmd.drive.steering_angle_velocity = 0
        ...
        while data is None:
            try:
                data_imu = rospy.wait_for_message('/TFF/arduino_imu', Imu, timeout=5)
    ...
    def reset(self):
        ...
        vel_cmd = AckermannDriveStamped();
        vel_cmd.drive.speed = 0;
        vel_cmd.drive.acceleration = 0;
        vel_cmd.drive.jerk = 0;
        vel_cmd.drive.steering_angle = 0
        vel_cmd.drive.steering_angle_velocity = 0
        ...
        while data is None:
            try:
                print("DDPG Ready ...")
                data_green = rospy.wait_for_message('/TFF/GREEN_Start_DDPG', Bool, timeout=5)
                data_imu = rospy.wait_for_message('/TFF/arduino_imu', Imu, timeout=5)
                data_cam = rospy.wait_for_message('/TFF/ddpg_camera_input', Float32MultiArray, timeout=5)

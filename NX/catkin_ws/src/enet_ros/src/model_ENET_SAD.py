...
#        self.fc_input_feature = 3 * int(input_w / 16) * int(input_h / 16) #
        self.fc_input_feature = 2 * int(input_w / 16) * int(input_h / 16) #
#        self.num_classes = 3 if dataset != 'BDD100K' else 1 #
        self.num_classes = 2 if dataset != 'BDD100K' else 1 #
        ...
        if dataset != 'BDD100K':
#            self.ce_loss = nn.CrossEntropyLoss(weight=torch.tensor([self.scale_background, 1, 1])) #
            self.ce_loss = nn.CrossEntropyLoss(weight=torch.tensor([self.scale_background, 1])) #
            self.bce_loss = nn.BCELoss()
#            self.iou_loss = mIoULoss(n_classes=2) #
        ...
        # Lane exist (P1)
#        self.exist = nn.Sequential(nn.Conv2d(128, 3, 1),
        self.exist = nn.Sequential(nn.Conv2d(128, 2, 1),
                                   nn.Softmax(dim=1),
                                   nn.AvgPool2d(2, 2),)#
...
#                    seg_gt_onehot = to_one_hot(seg_gt, 3)#
                    seg_gt_onehot = to_one_hot(seg_gt, 2)#
                else:
...
if __name__ == '__main__':
    now = time.time()
    print(now)
    tensor = torch.ones((2, 3, 360, 480)).cuda()
    seg_gt = torch.zeros((2, 360, 480)).long().cuda()
    exist_gt = torch.ones((2, 2)).cuda()
    no_now = time.time()
    print(no_now - now)
    enet_sad = ENet_SAD((480, 360), sad=True, dataset='BDDK')
    enet_sad.cuda()
    nono_now = time.time()
    print(nono_now - no_now)
    enet_sad.train(mode=True)
    result = enet_sad(tensor, seg_gt, exist_gt, sad_loss=True)
    nonono_now = time.time()
    print(nonono_now - nono_now)
    print(result)

...
def seg_img_callback(data):
    global cv_seg_image
    # 카메라로부터 받은 img 데이터를 OpenCV로 처리를 할 수 있게 변환해주는 함수 사용
    cv_seg_image = bridge.imgmsg_to_cv2(data, "rgb8")
    #cv_seg_image = cv2.resize(cv_seg_image, dsize=(0, 0), fx=1, fy=4, interpolation=cv2.INTER_LINEAR)
    cv_seg_image = cv2.resize(cv_seg_image, dsize=(480, 360), interpolation=cv2.INTER_AREA)
...
rospy.Subscriber("/carla/ego_vehicle/camera/semantic_segmentation/front/image_segmentation", Image, seg_img_callback)

  
while not rospy.is_shutdown():
    #if cv_detect_image.size != (368*640*3):
    if cv_detect_image.size != (360*480*3):
        continue
        
    cv2.namedWindow('detect_image')
    cv2.imshow("detect_image", cv_detect_image)
    cv2.namedWindow('seg_image')
    cv2.imshow("seg_image", cv_seg_image)

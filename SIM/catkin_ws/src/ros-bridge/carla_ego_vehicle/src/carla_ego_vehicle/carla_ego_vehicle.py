...
class CarlaEgoVehicle(object):
    ...
    def on_initialpose(self, initial_pose):
        ...
        """
        print("---"*10)
        print("set initial pose", self.num)
        self.num += 1
        #print(initial_pose.pose.pose)
    ...
    def restart(self):
        ...
        #print("-----------------blueprint-------------------")
        #print(self.world.get_blueprint_library().filter(self.actor_filter))
        blueprint = self.world.get_blueprint_library().filter(self.actor_filter)[3] #s, police car

        blueprint.set_attribute('role_name', "{}".format(self.role_name))
        if blueprint.has_attribute('color'):
            color = secure_random.choice(blueprint.get_attribute('color').recommended_values) #s, random color
            #print("-------------------color-----------------------")
